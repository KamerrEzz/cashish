...
export function registerCashishTools(server: McpServer, userId: string) {
  ...
  server.registerTool(
    ...
    async ({ days }) => {
      ...
      return ok({ from: today, to: endStr, events });
    },
  );
  ...
  server.registerTool(
    ...
    async ({ account_id }) => {
      const supabase = db();
      try {
        const account = await getOwnedAccount(supabase, userId, account_id);
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      try {
        ...
        return fail(e instanceof Error ? e.message : "Saldo inválido");
      }
      ...
      if (input.type === "credit_card") {
        ...
        try {
          ...
          return fail(e instanceof Error ? e.message : "Límite inválido");
        }
        if (openingCents < 0) return fail("La deuda inicial no puede ser negativa");
        ...
        const { data: account, error } = await supabase
          ...
          .select("*")
          .single();
        if (error || !account) return fail(error?.message ?? "No se creó la cuenta");
        ...
        return ok({ account, open_statement: period });
      }
  ...
  server.registerTool(
    ...
    async ({ account_id, type, from_date, to_date, limit }) => {
      let query = db()
        .from("transactions")
        .select("*, accounts(name)")
        .eq("user_id", userId)
  ...
  server.registerTool(
    ...
    async ({ query: q, account_id, limit }) => {
      const pattern = `%${q}%`;
      let query = db()
        .from("transactions")
        .select("*, accounts(name)")
  ...
  server.registerTool(
    ...
    async ({ transaction_id }) => {
      const { data, error } = await db()
        ...
        .maybeSingle();
      if (error) return fail(error.message);
      if (!data) return fail("Movimiento no encontrado");
      return ok(data);
    },
  );
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      if (input.from_account_id === input.to_account_id) {
        return fail("Las cuentas de origen y destino deben ser distintas");
      }
      ...
      try {
        ...
        if (fromAcc.type === "credit_card") {
          return fail("No se puede transferir desde una TDC en el MVP");
        }
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      try {
        ...
        if (fromAcc.type === "credit_card") {
          return fail("No se puede pagar desde una TDC");
        }
        if (toAcc.type !== "credit_card") {
  ...
  server.registerTool(
    ...
    async ({ account_id }) => {
      ...
      const { data: period, error } = await supabase
        ...
        .maybeSingle();
      if (error) return fail(error.message);
      if (!period) return fail("No hay periodo abierto para esta tarjeta");
  ...
  server.registerTool(
    ...
    async ({ account_id, minimum_payment }) => {
      const supabase = db();
      try {
        const account = await getOwnedAccount(supabase, userId, account_id);
        if (account.type !== "credit_card") {
          return fail("Solo aplica a tarjetas de crédito");
        }
        ...
        const { data: profile } = await supabase
          ...
          .eq("user_id", userId)
          .single();
        if (!profile) return fail("Perfil de TDC no encontrado");
        ...
        if (minimum_payment != null && minimum_payment.trim() !== "") {
          try {
            ...
            return fail(e instanceof Error ? e.message : "Pago mínimo inválido");
          }
        }
        ...
        const { data: opened, error: openError } = await supabase
          ...
          .select("*")
          .single();
        if (openError) return fail(openError.message);
  ...
  server.registerTool(
    ...
    async ({ subscription_id }) => {
      const { data, error } = await db()
        ...
        .maybeSingle();
      if (error) return fail(error.message);
      if (!data) return fail("Suscripción no encontrada");
      return ok(data);
    },
  );
  ...
  server.registerTool(
    ...
    async (input) => {
      ...
      try {
        await getOwnedAccount(db(), userId, input.account_id);
      } catch (e) {
        return fail(e instanceof Error ? e.message : "Cuenta inválida");
      }
      const { data, error } = await db()
        .from("subscriptions")
        .insert({
  ...
  server.registerTool(
    ...
    async ({ subscription_id, is_active }) => {
      const { data, error } = await db()
        .from("subscriptions")
        .update({
          is_active,
